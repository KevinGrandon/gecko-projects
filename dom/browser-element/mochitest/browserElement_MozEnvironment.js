...
// Test that the onmozbrowsermetachange event for moz-environment works.
"use strict";
...
function runTest() {
  ...
  iframe1.src = "http://test/tests/dom/browser-element/mochitest/file_browserElement_MozEnvironment.html";
  ...
  function tests(e) {
    ...
    switch (numMetaChanges++) {
      case 0: {
        is(detail.name, 'moz-environment', 'name matches');
        is(detail.content, 'immersive', 'content matches');
      ...
      case 1: {
        is(detail.name, 'moz-environment', 'name matches');
        is(detail.content, 'default', 'content matches');
